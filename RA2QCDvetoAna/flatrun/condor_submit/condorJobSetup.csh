...
set mainDir = "/share/store/users/samantha/CMSSW_5_2_5/src/UserCode/RA2QCDvetoAna/flatrun/condor_submit/02082013_QCDMC_SeedTest_odd_1k/${subdir}"
...
foreach dir ( $dirlist )
	...
	if ( ! -d $dir ) then
		...
		if ( $i == 1 ) then
			...
			./fnalCondorSubmit.csh 7 1 $fileList $systVariation 
			cd $curDir
			#exit
		else if ( $i == 2 ) then
			...
			set fileList =  `ls -1 *.files`
			echo "$PWD -> $fileList"
			./fnalCondorSubmit.csh 2 1 $fileList $systVariation 
			cd $curDir
		endif
