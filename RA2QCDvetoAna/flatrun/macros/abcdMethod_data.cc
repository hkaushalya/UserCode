...
double abcdMethod_data::GetCerrValue(const int jetbin_low, const int jetbin_hi, const double x)
{

	vector<pair<double,double> > binErr; //low bin edge->error

	if (jetbin_low == 3 && jetbin_hi==5)
	{
		//cout << __FUNCTION__ << ": using 3-5" << endl; 
		//Syst for Njet3to5: 14 bins
		binErr.push_back(make_pair(50.0, 0.0632282));
		binErr.push_back(make_pair(70, 0.0460492));
		binErr.push_back(make_pair(90, 0.0441716));
		binErr.push_back(make_pair(110, 0.0362316));
		binErr.push_back(make_pair(130, 0.0183909));
		binErr.push_back(make_pair(150, 0.00558224));
		binErr.push_back(make_pair(190, 0.00236755));
		binErr.push_back(make_pair(230, 0.00140264));
		binErr.push_back(make_pair(270, 0.00269221));
		binErr.push_back(make_pair(370, 0.00875138));
		binErr.push_back(make_pair(520, 0.00978027));
		binErr.push_back(make_pair(670, 0.00570672));
		binErr.push_back(make_pair(820, 0.00213177));

	} else if (jetbin_low == 6 && jetbin_hi==7)
	{
		//Syst for Njet6to7: 14 bins

		binErr.push_back(make_pair(50, 0.0260485));
		binErr.push_back(make_pair(70, 0.0197466));
		binErr.push_back(make_pair(90, 0.0526808));
		binErr.push_back(make_pair(110, 0.0744995));
		binErr.push_back(make_pair(130, 0.0591962));
		binErr.push_back(make_pair(150, 0.0252379));
		binErr.push_back(make_pair(190, 0.0173188));
		binErr.push_back(make_pair(230, 0.0109202));
		binErr.push_back(make_pair(270, 0.0204205));
		binErr.push_back(make_pair(370, 0.0477553));
		binErr.push_back(make_pair(520, 0.100913));
		binErr.push_back(make_pair(670, 0.0389791));
		binErr.push_back(make_pair(820, 0.0787221));
	} else if (jetbin_low >= 8)
	{
		//Syst for Njet8to1000: 14 bins

		binErr.push_back(make_pair(50, 0.107631));
		binErr.push_back(make_pair(70, 0.0916267));
		binErr.push_back(make_pair(90, 0.0930659));
		binErr.push_back(make_pair(110, 0.127377));
		binErr.push_back(make_pair(130, 0.104878));
		binErr.push_back(make_pair(150, 0.0688899));
		binErr.push_back(make_pair(190, 0.0324988));
		binErr.push_back(make_pair(230, 0.051854));
		binErr.push_back(make_pair(270, 0.102605));
		binErr.push_back(make_pair(370, 0.19744));
		binErr.push_back(make_pair(520, 0.62552));
		binErr.push_back(make_pair(670, 0.37947));
		binErr.push_back(make_pair(820, 0.439403));
	} else {
		cout << __FUNCTION__ << ": Unspecified JET bins. No syst found for 'c' !" << endl;
		assert (false);
	...
	if (x<binErr.at(0).first) c = binErr.at(0).second;
	else if (x>binErr.at(binErr.size()-1).first) c = binErr.at(binErr.size()-1).second;
	else 
	{
		for (unsigned bin = 0; bin < binErr.size() - 1 ; ++bin)
		{
			//cout << "x/lo/hi = " << x << "/" << binErr.at(bin).first << "/" << binErr.at(bin+1).first << endl;
			if(x >= binErr.at(bin).first && x < binErr.at(bin+1).first)
			{
				//cout << "bin/val/x = " << binErr.at(bin).first << "/" << binErr.at(bin).second << "/" << x << endl;
				c= binErr.at(bin).second;
				break;
			}
		}
	}
	
	//cout << __FUNCTION__ << ": NO C-SYST found for this bin!!! can be right!!!" << endl;
	//assert (false);
...
abcdMethod_data::abcdMethod_data(const string bgFileName)
{

	Nsyst = 1;  // c-syst, sideband syst/c_additional
	b_doSystematics = true;
	//numHistName = "reco_failFineBin1";
	//denHistName = "reco_signalFineBin";
	numHistName = "reco_signal";
	denHistName = "reco_fail1";
	controlHistName = "reco_failFineBin1";
	signalHistName = "reco_signalFineBin";
	controlsyst1HistName = "reco_sidebandSyst1_fineBin";
	...
	bgRootFileName = bgFileName;
}

abcdMethod_data::~abcdMethod_data()
{
	vg_Inputs.clear();
...
void abcdMethod_data::SubtractBackgrounds(TH1* h, const string histname, const bool bUseHtMerged)
{
	cout << __FUNCTION__ << "bUseHtMerged= " << bUseHtMerged << endl;
	TH1* bghist = 0;

	if (bUseHtMerged) bghist = (TH1*) bgHTmergedRootFile->Get(histname.c_str());
	else  bghist = (TH1*) bgRootFile->Get(histname.c_str());

	if (bghist == 0)
	{
		cout << __FUNCTION__ << ": background hist " << histname << " is not found in BG hist ";
		if (bUseHtMerged) cout << bgHTmergedRootFile->GetName();
		else cout << bgRootFile->GetName();
		
		cout << endl;
		assert(false);
	}
	const double int_b4 = h->Integral();
	const double int_bg = bghist->Integral();
	h->Add(bghist, -1);
	const double int_a4 = h->Integral();

	cout << __FUNCTION__ << ": b4/a4/bg= " << int_b4 << "/" << int_a4 << "/" << int_bg << endl;
}

double abcdMethod_data::GetCsyst(TH1* h, const int jetbin_lo, const int jetbin_hi, const double mht_min, const double mht_max)
{
	double pred = 0;
	for (int bin=1; bin<=h->GetNbinsX(); ++bin)
	{
		const double bin_center = h->GetBinCenter(bin);
		//calculate presiction for this particular bin
		if (bin_center >= mht_min && bin_center < mht_max)
		{
			const double val = h->GetBinContent(bin);
			if (val>0)
			{
				const double f  = GetCerrValue(jetbin_lo, jetbin_hi, bin_center);
				//cout << __FUNCTION__ << " val / f: " << val << "/" << f << endl;
				pred += f *val;
			}
		}

	}
	cout << __FUNCTION__ << ": pred = " << pred << endl;
	return pred;
}

TH1* abcdMethod_data::GetRatioHist(TFile* f, const string path_to_hist, const bool debug)
{
	...
	//} else {
	//	numHistPath << path_to_hist << "/reco_signal";
	//	rebin = 2;
	//}
	...
	const bool bUseHtMerged = 1;
	SubtractBackgrounds(h_num, numHistPath.str(), bUseHtMerged);
	SubtractBackgrounds(h_den, denHistPath.str(), bUseHtMerged);
...
void abcdMethod_data::AddInput(const string rootFileName,const string path_to_hist, const string legendText,
						const string searchbins, const float c_value, const bool debug)
{
	
	input_t input;
		
	input.file = new TFile (rootFileName.c_str());
//	TFile *f = new TFile ("01292013_data_nodphi_trigsel_trigwgt_noBGsub/qcd_all.root");
//	if (f->IsZombie()) {
//		cout << "File with name 01292013_data_nodphi_trigsel_trigwgt_noBGsub/qcd_all.root  not found!" << endl;
//		assert(false);
//	}
	if (input.file->IsZombie()) {
		cout << "File with name " << rootFileName << " not found!" << endl;
		assert(false);
	}

	bgRootFile = new TFile (bgRootFileName.c_str());
	if (bgRootFile->IsZombie()) {
		cout << "File with name " << bgRootFileName << " not found!" << endl;
		assert(false);
	}
	bgHTmergedRootFileName = "TotBG_MCpuWgted_Lumi_12187pb_HTranged.root";
	bgHTmergedRootFile = new TFile (bgHTmergedRootFileName.c_str());
	if (bgHTmergedRootFile->IsZombie()) {
		cout << "File with name " << bgHTmergedRootFileName << " not found!" << endl;
		assert(false);
	}
...
//	GetControlHist(input.file, input.hist_control_finebin, input.njetrange, input.htrange, input.mhtrange, path_to_hist, debug);  
	TFile *fsigcon = new TFile("01292013_data_nodphi_trigsel_trigwgt_noBGsub/qcd_all.root");
	if (fsigcon->IsZombie()) {
		cout << __FUNCTION__ << ": control/signal root file qcd_all.root is not found!! " << endl;
		assert(false);
	}
...
void abcdMethod_data::Run()
{
	...
	for (unsigned i=0; i < vg_Inputs.size(); ++i)
	{
		...
		legText <<  "Data(A+B+C): " << str_njetrange.str();
		...
		if (b_doSystematics)
		{
			if (bgRootFile->IsZombie())
			{
				cout << __FUNCTION__ << ": Background ROOT file with name " << bgRootFileName << " not found!" << endl;
				assert(false);
			}
			for (int s=0; s <Nsyst; ++s)
			{
				...
				//		vg_Inputs.at(i).Systs.at(s).c_value,
				//		systtype.str() );
			}
		}
		...
		{
			const int jetbin_lo = vg_Inputs.at(i).njetrange.at(jetbin).first;
			const int jetbin_hi = vg_Inputs.at(i).njetrange.at(jetbin).second;
			for (unsigned htbin = 0; htbin < vg_Inputs.at(i).htrange.size(); ++htbin)
			{
				const int htbin_lo = vg_Inputs.at(i).htrange.at(htbin).first;
				const int htbin_hi = vg_Inputs.at(i).htrange.at(htbin).second;
				...
				for (unsigned mhtbin = 0; mhtbin < vg_Inputs.at(i).mhtrange.size(); ++mhtbin)
				{
					...
					if (b_doSystematics)
					{
						...
						const double alt_pred = GetCsyst(vg_Inputs.at(i).hist_control_finebin.at(jetbin).at(htbin), jetbin_lo, jetbin_hi, mht_min, mht_max);
						cout << jetbin_lo << "-" << jetbin_hi << ":" << htbin_lo << "-" << htbin_hi << ":" << mht_min << "-" << mht_max 
								<< " = "<< cen_pred_gaus << " | " <<  alt_pred << endl;
						//do not need to deduct from cen value as what I am using as c-syst is the sqrt of central-val substracted quadratures		
						//c_systTot_gaus2 += pow( fabs(cen_pred_gaus - alt_pred), 2); 
						//c_systTot_expo2 += pow( fabs(cen_pred_expo - alt_pred), 2); 
						c_systTot_gaus2 += alt_pred; 
						c_systTot_expo2 += alt_pred; 
...
void abcdMethod_data::GetFits(TH1* ratio_hist, 
...
void abcdMethod_data::GetPredictions(const float& min_mht, const float& max_mht, 
...
void abcdMethod_data::PrintRatioCanvas()
{
	cout << "Printing ratio canvas.. " << endl;
	canvas_for_ratios->cd();
	canvas_for_ratios->Print(epsfile_for_ratios.c_str());
}
void abcdMethod_data::PrintDebugCanvas()
{
...
void abcdMethod_data::PrintResults(vector< pair<float, float> >& njetrange,
...
void abcdMethod_data::SetSearchBinInfo( 
...
void abcdMethod_data::GetSignalHist(TFile *f, vector< vector<TH1*> > & signalHists, 
										vector< pair<float, float> >& njetrange,
										vector< pair<float, float> >& htrange,
										vector< pair<float, float> >& mhtrange,
										const string path_to_hist, const bool debug)
{
	...
	for (unsigned jetbin = 0; jetbin < njetrange.size(); ++jetbin)
	{
		...
		for (unsigned htbin = 0; htbin < htrange.size(); ++htbin)
		{
			...
			const string temp_signal_hist_name ("reco_signalFineBin");
			...
			if (s_hist == NULL)
			{
				...
				hist->SetDirectory(0); 
				SubtractBackgrounds(hist, signalHist.str(),0);
...
//will need to merge these two method_datas. for now separate because of naming issues
//in the smear_signal (should be reco_signal

void abcdMethod_data::GetControlHist(TFile *f, vector< vector<TH1*> > & controlHists, 
										vector< pair<float, float> >& njetrange,
										vector< pair<float, float> >& htrange,
										vector< pair<float, float> >& mhtrange,
										const string path_to_hist, const bool debug)
{
	...
	for (unsigned jetbin = 0; jetbin < njetrange.size(); ++jetbin)
	{
		...
		for (unsigned htbin = 0; htbin < htrange.size(); ++htbin)
		{
			...
			if (s_hist == NULL)
			{
				cout << __FUNCTION__ << ": Control hist(" << histname.str() << ") not found in  file "
					<< f->GetName() << endl;
				assert (false);
			} else { 
				TH1* hist = dynamic_cast<TH1*> (s_hist->Clone());
				...
				hist->SetDirectory(0); 
				SubtractBackgrounds(hist, histname.str(),0);
...
void abcdMethod_data::InitSysts(
