...
double abcdMethod_MC::GetCerrValue(const int jetbin_low, const int jetbin_hi, const double x)
{
...
double abcdMethod_MC::GetCsyst(TH1* h, const int jetbin_lo, const int jetbin_hi, const double mht_min, const double mht_max)
{
	double pred = 0;
...
TH1* abcdMethod_MC::GetRatioHist(TFile* f, const string path_to_hist, const bool debug)
{
...
void abcdMethod_MC::AddInput(const string rootFileName,const string path_to_hist, const string legendText,
...
void abcdMethod_MC::Run()
{
	...
	for (unsigned i=0; i < vg_Inputs.size(); ++i)
	{
		...
		const int nj_min = vg_Inputs.at(i).njetrange.at(0).first; 
		const int nj_max = vg_Inputs.at(i).njetrange.at(0).second;

		if (nj_max> 100) 	str_njetrange <<  "#geq " << nj_min << " Jets "; 
		else  str_njetrange <<  nj_min << "-" << nj_max << " Jets "; 

		
		//legText <<  str_njetrange.str() <<  ": Default conditions for 'C'";
		legText <<  str_njetrange.str() <<  ": Mean value of constant `c' ";
...
void abcdMethod_MC::GetFits(TH1* ratio_hist, 
...
void abcdMethod_MC::GetPredictions(const float& min_mht, const float& max_mht, 
...
void abcdMethod_MC::PrintRatioCanvas()
{
	cout << "Printing ratio canvas.. " << endl;
	canvas_for_ratios->cd();
	canvas_for_ratios->Print(epsfile_for_ratios.c_str());
}
void abcdMethod_MC::PrintDebugCanvas()
{
...
void abcdMethod_MC::PrintResults(vector< pair<float, float> >& njetrange,
										vector< pair<float, float> >& htrange,
										vector< pair<float, float> >& mhtrange,
const vector< vector<vector< pair<abcdMethod_MC::Predictions_t, abcdMethod_MC::Predictions_t> > > >& results)
{
	cout <<  "\tNJET""\tHT "
					<< setw(15) << "MHT"
					<< setw(15) << " mean " 
...
void abcdMethod_MC::SetSearchBinInfo( 
			vector< pair<float, float> >& njetrange,
			vector< pair<float, float> >& htrange,
			vector< pair<float, float> >& mhtrange,
			vector< vector<vector< pair<abcdMethod_MC::Predictions_t, abcdMethod_MC::Predictions_t> > > >& results, 
...
void abcdMethod_MC::GetSignalHist(TFile *f, vector< vector<TH1*> > & signalHists, 
										vector< pair<float, float> >& njetrange,
										vector< pair<float, float> >& htrange,
										vector< pair<float, float> >& mhtrange,
...
//will need to merge these two method_MCs. for now separate because of naming issues
//in the smear_signal (should be smeared_signal

void abcdMethod_MC::GetControlHist(TFile *f, vector< vector<TH1*> > & controlHists, 
										vector< pair<float, float> >& njetrange,
										vector< pair<float, float> >& htrange,
										vector< pair<float, float> >& mhtrange,
										const string path_to_hist, const bool debug)
...
void abcdMethod_MC::InitSysts(
			//vector< vector< vector< vector< abcdMethod_MC::Syst_t > > > > & systs, 
			vector< abcdMethod_MC::Syst_t > & systs, 
			vector< pair<float, float> >& njetrange,
			vector< pair<float, float> >& htrange,
			vector< pair<float, float> >& mhtrange,
			const string path_to_hist, const bool debug)
