...
// $Id: Factorization.cc,v 1.3 2011/10/17 22:24:27 samantha Exp $
//
//
...
Factorization::Factorization(const edm::ParameterSet& iConfig)
{
   //now do what ever initialization is needed
	...
	dMinHT = iConfig.getUntrackedParameter<double>("dMinHT",600.0);
...
//	pf30_jet3Hist.delphi = fs->make<TH1F> ("pf30_jet3_delphi" ,"RA2: PF30-Jet3: delphi;delphi;Events;", 160, -8, 8);
	...
	hFail[6] = fs->make<TH1F> ("Fail_lt_point2","Fail from #Delta#Phi_{min} cut <0.2", npassFailHistBins, passFailHistBins);
	hFail[7] = fs->make<TH1F> ("Fail_lt_point3","Fail from #Delta#Phi_{min} cut <0.3", npassFailHistBins, passFailHistBins);
	hFail[8] = fs->make<TH1F> ("Fail_lt_point2_HT500","Fail from #Delta#Phi_{min} cut <0.2 && HT>500 GeV", npassFailHistBins, passFailHistBins);
	hFail[9] = fs->make<TH1F> ("Fail_lt_point2_HT800","Fail from #Delta#Phi_{min} cut <0.2 && HT>800 GeV", npassFailHistBins, passFailHistBins);
	...
	hSignalRegion[0] = fs->make<TH1F> ("Signal_0","Signal Region: MHT>200 GeV && pass RA2 #Delta#Phi_{min}", npassFailHistBins, passFailHistBins);
	hSignalRegion[1] = fs->make<TH1F> ("Signal_1","Signal Region: MHT>350 GeV && pass RA2 #Delta#Phi_{min}", npassFailHistBins, passFailHistBins);
	hSignalRegion[2] = fs->make<TH1F> ("Signal_2","Signal Region: MHT>500 GeV && pass RA2 #Delta#Phi_{min}", npassFailHistBins, passFailHistBins);

	hSignalRegion[0]->Sumw2();
	hSignalRegion[1]->Sumw2();
	hSignalRegion[2]->Sumw2();

	hDelPhiMin_mht[0] = fs->make<TH1F> ("ra2_delPhiMin_MHT_60TO80","RA2: #Delta#Phi_{min} for 60 <#slash{H}_{T}<80 GeV", 150, 0, 3);
	hDelPhiMin_mht[1] = fs->make<TH1F> ("ra2_delPhiMin_MHT_80to100","RA2: #Delta#Phi_{min} for 80 <#slash{H}_{T}<100 GeV", 150, 0, 3);
	hDelPhiMin_mht[2] = fs->make<TH1F> ("ra2_delPhiMin_MHT_100to120","RA2: #Delta#Phi_{min} for 100<#slash{H}_{T}<120 GeV", 150, 0, 3);
	hDelPhiMin_mht[3] = fs->make<TH1F> ("ra2_delPhiMin_MHT_120to140","RA2: #Delta#Phi_{min} for 120<#slash{H}_{T}<140 GeV", 150, 0, 3);
	hDelPhiMin_mht[4] = fs->make<TH1F> ("ra2_delPhiMin_MHT_140to170","RA2: #Delta#Phi_{min} for 140<#slash{H}_{T}<170 GeV", 150, 0, 3);
	hDelPhiMin_mht[5] = fs->make<TH1F> ("ra2_delPhiMin_MHT_170to200","RA2: #Delta#Phi_{min} for 170<#slash{H}_{T}<200 GeV", 150, 0, 3);
	hDelPhiMin_mht[6] = fs->make<TH1F> ("ra2_delPhiMin_MHT_200to250","RA2: #Delta#Phi_{min} for 200<#slash{H}_{T}<250 GeV", 150, 0, 3);
	hDelPhiMin_mht[7] = fs->make<TH1F> ("ra2_delPhiMin_MHT_250up","RA2: #Delta#Phi_{min} for #slash{H}_{T}>250 GeV", 150, 0, 3);
				...
				)
{
	...
	for (unsigned i = 0 ; i < pfpt50eta25JetHandle->size() ; ++i)
	{
		...
		if (i>2) break; //use only three leasing jets
	...
	if (dPhiMin>0.2) hPass[1]->Fill(mht, Weight);
	else 	hFail[1]->Fill(mht, Weight);

	if (dPhiMin>0.25) hPass[2]->Fill(mht, Weight);
	else 	hFail[2]->Fill(mht, Weight);

	if (dPhiMin>=0.3) hPass[3]->Fill(mht, Weight);
	else 	hFail[3]->Fill(mht, Weight);
	...
	if (passed) 
	{
		...
		if ( (*htHandle) > 500)
		{
			...
			if (dPhiMin<0.2) hFail[8]->Fill(mht, Weight);
		}
		if ( (*htHandle) > 800) 
		{
			hPass[8]->Fill(mht, Weight);
			if (dPhiMin<0.2) hFail[9]->Fill(mht, Weight);
		}
