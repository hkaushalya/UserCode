...
// The functions driving the topTagger procedure
         ...
         void findDiTopCandidates(const std::vector<TLorentzVector> &oriJetsVec);
...
// The very basic function faciliating the triplet making and variable calculation
         ...
         void onlyfatJetsCloseInAClone(const vector<int> &indexVec, const vector<TLorentzVector> &oriJetsVec, vector<vector<int> > &tmpFinalCombfatJets, vector<vector<int> > &tmpFinalRemaining, vector<vector<vector<int> > > &tmpFinalCombJetSubStruc);
...
// Transfer the fat jets to pt ordered
         ...
         void prepareFindingBestTopCandidate(const std::vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS){
            ...
            onlyfatJetsCloseInAClone(indexVec, oriJetsVec, finalCombfatJets, finalRemaining, finalCombJetSubStruc);
         ...
         // Then also the remaining jets contain at least one.
               ...
               if( cntnbJetsCSVS >=2 && cntTopCandPassCSVS == 0 ) continue;
         ...
         void runOnlyTopTaggerPart(const std::vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS){
            ...
            CSVS_ = -1e10; maxEtaForbJets_ = 99.0;
            ...
            CSVS_ = cachedCSVS; maxEtaForbJets_ = cachedmaxEtaForbJets;
...
// in order for the following procedure.
         ...
         void prepareCutVariables(const std::vector<TLorentzVector> &oriJetsVec, const std::vector<double> &recoJetsBtagCSVS, const TLorentzVector &metLVec){
            ...
            mTbestWJet = calcMT(bestCloseTomWLVec, metLVec);

            mTbestbJet = calcMT(bestbJetLVec, metLVec);
...
// For di-Top cases
// recoJetsBtagCSVS: is NOT used now.
...
// The tunning parameters
         ...
         void setCSVS( const double CSVS ){ CSVS_ = CSVS; globalCachedCSVS_ = CSVS_; }
         void setnSubJetsDiv( const int nSubJetsDiv ){ nSubJetsDiv_ = nSubJetsDiv; }
         void setnJetsSel ( const int nJetsSel ){ nJetsSel_ = nJetsSel; }
         void setmaxEtaForbJets( const double maxEtaForbJets ){ maxEtaForbJets_ = maxEtaForbJets; }
         ...
         void setdobVetoCS( const bool dobVetoCS ){
            ...
            if( dobVetoCS_ ){
               CSVS_ = -1e10; globalCachedCSVS_ = CSVS_; 
