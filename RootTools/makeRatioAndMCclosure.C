...
void makeRatioAndMCclosure(const string numeHistName, const string denoHistName, 
			...
			const float fitrange_xmin = 50, const float fitrange_xmax = 150) 
{
	...
	const float C_UPLIMIT = 0.09;;
	const float C_LOLIMIT = 0.005;;
	...
	expFit_sigma->SetParameter(2,expFit->GetParameter(2));
	...
	gausFit_sigma->SetParameter(2,gausFit->GetParameter(2));
...
void makeRatioAndMCclosure(const string njetlabel= "XX")
{
...
//			for (unsigned j = 0; j < htbinlabels.size(); ++j)
			{
				...
				makeRatioAndMCclosure(numeHistName.str(), denoHistName.str(), title.str(), htbinlabels.at(i), signalHistName.str(), controlHistName.str()); 
			}
		}
	}
	c->Print("ratios.eps]");
